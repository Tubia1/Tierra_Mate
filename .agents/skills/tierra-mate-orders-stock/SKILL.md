---
name: tierra-mate-orders-stock
description: Diseñar, implementar, corregir o revisar pedidos, carrito por WhatsApp, reservas temporales, variantes, disponibilidad, confirmación de ventas y movimientos de inventario de Tierra Mate Shop.
---
...
- Verificar si existen endpoints, servicios y pruebas; una tabla no implica que el flujo esté implementado.
...
- Crear la reserva justo antes de abrir WhatsApp y mantenerla 30 minutos, usando `store_settings.reservation_minutes` como fuente configurable.
...
- Bloquear las variantes necesarias con `SELECT ... FOR UPDATE`, siempre en orden de ID para reducir bloqueos cruzados.
- Recalcular dentro de la transacción las reservas con estado `reserved` y `reserved_until > now()`.
- Rechazar el pedido completo si alguna variante no tiene disponibilidad.
- Copiar en `order_items` nombre, variante, SKU y precio actuales para preservar el historial; recalcular totales en el servidor.
